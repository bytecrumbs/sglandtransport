...
final appDatabaseProvider = Provider<AppDatabase>(AppDatabase.new);
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    beforeOpen: (details) async {
      ...
      final refreshDate =
          await ref.read(localStorageServiceProvider).getInt(refreshDateKey) ??
          0;
      ...
      if (details.wasCreated ||
        ...
        ref.watch(dBInitProvider.notifier).initDBStart();
        ...
        ref.watch(dBInitProvider.notifier).initDBComplete();
      }
    },
  );
  ...
  Future<void> _refreshBusRoutes() async {
    ...
    for (var i = 0; i <= 26000; i = i + 500) {
      ...
      ref
          .watch(dBInitProvider.notifier)
          .busRoutesLoadingUpdate(percentComplete);
    }
    ref.watch(dBInitProvider.notifier).busRoutesLoadingComplete();
  }

  Future<void> _refreshBusServices() async {
    ref.read(loggerProvider).d('deleting bus services from table');
    ref.watch(dBInitProvider.notifier).busServicesLoadingUpdate(0);
  ...
  Future<void> _refreshBusStops() async {
    ...
    for (var i = 0; i <= 5000; i = i + 500) {
      ...
      ref.watch(dBInitProvider.notifier).busStopsLoadingUpdate(percentComplete);
    }
    ref.watch(dBInitProvider.notifier).busStopsLoadingComplete();
  }
  ...
  Future<List<BusStopValueModel>> getBusStops({
    ...
    final tableBusStopList = await (select(
      tableBusStops,
    )..where((tbl) => tbl.busStopCode.isIn(busStopCodes))).get();
  ...
  Future<List<BusStopValueModel>> findBusStops(String searchTerm) async {
    ...
    final tableBusStopList =
        await (select(tableBusStops)..where(
              (tbl) =>
  ...
  Future<List<BusServiceValueModel>> getBusService({
    ...
    final tableBusServicesList =
        await (select(tableBusServices)..where(
              (tbl) =>
  ...
  Future<List<BusRouteWithBusStopInfoModel>> getBusRoute({
    ...
    /// to load it again
    final checkForBusRouteAvailability =
        await (select(tableBusRoutes)..where(
              (t) =>
    ...
    final tableBusRouteList =
        (select(tableBusStops).join([
            innerJoin(
              tableBusRoutes,
            ...
            tableBusRoutes.serviceNo.equals(serviceNo) &
                tableBusRoutes.direction.equalsExp(
                  subqueryExpression(direction),
                ) &
                tableBusRoutes.stopSequence.isBiggerOrEqual(
                  subqueryExpression(stopSequence),
                ),
          ))
